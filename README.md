...
│   └── ENV_LOADING.md  # 📄 Документация по автоматической загрузке .env
└── utils/           # Утилиты проекта
    └── envLoader.js    # 🔧 Автоматический поиск и загрузка .env файла
```
...
**🎉 Новая функция!** Проект теперь автоматически находит файл `.env`, начиная с текущей директории и поднимаясь вверх по дереву папок до корня проекта.

Это означает, что:

- ✅ Не нужно указывать абсолютные пути к `.env` файлу
- ✅ Проект будет работать на любом компьютере без изменения кода
- ✅ `.env` файл автоматически найдется в корне проекта `testKeyfory/`

📚 **Подробная документация:** [docs/ENV_LOADING.md](docs/ENV_LOADING.md)

### Размещение .env файла

Разместите файл `.env` в корневой папке проекта:
