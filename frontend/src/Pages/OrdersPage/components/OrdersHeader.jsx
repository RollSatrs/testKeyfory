    ...
    switch (currentStep) {
        ...
        return (
          <div>
            ...
            <Select
              ...
              onChange={(value) =>
                setOrderForm({ ...orderForm, executer_id: value })
              }
              style={{ width: "100%" }}
              size="large"
            >
              {executors.map((executor) => (
                <Select.Option key={executor.id} value={executor.id}>
                  {executor.name} ({executor.telegram_id}) - Рейтинг:{" "}
                  {executor.rating}/5
                </Select.Option>
              ))}
            </Select>
      ...
      case 2:
        return (
          <div>
            <h3 className="text-lg font-semibold mb-4">
              Шаг 3: Выберите материалы для заказа
            </h3>
            <div className="mb-4">
              <span className="text-green-600 font-medium">
                Выбрано материалов: {selectedMaterials.length} из{" "}
                {materials.length}
              </span>
            </div>
      ...
      case 3:
        return (
          <div>
            <h3 className="text-lg font-semibold mb-4">
              Шаг 4: Укажите цену и статус
            </h3>
            <Input
              placeholder="Цена заказа (₽)"
              value={orderForm.total_sum}
              onChange={(e) =>
                setOrderForm({ ...orderForm, total_sum: e.target.value })
              }
            ...
            <Select
              ...
              onChange={(value) =>
                setOrderForm({ ...orderForm, payment_status: value })
              }
              style={{ width: "100%" }}
              size="large"
            >
